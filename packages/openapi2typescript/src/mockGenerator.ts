...
import fs from 'fs';
import { extractData, prettierFile, writeFile } from './util';
...
const genMockFiles = async (mockFunction: string[]) => {
  return new Promise((resolve) => {
    const prettierFileFormate = () =>  prettierFile(` 
    // @ts-ignore
...
const mockGenerator = async ({ openAPI, mockFolder }: genMockDataServerConfig) => {
  ...
  for (const file in mockActionsObj) {
    ...
    const filePath = extractData(file)
    try {
      let content = mockActionsObj[file];
      if (content) {
        content = await genMockFiles(content);
        content && (await writeFile(mockFolder + `/${filePath}`, `${file}.mock.ts`, content));
      }
    } catch (error) {
      console.log(error);
      throw error;
