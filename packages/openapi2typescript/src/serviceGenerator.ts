...
import Log from './log';
import { extractData, stripDot, writeFile } from './util';
...
class ServiceGenerator {
  ...
  constructor(config: GenerateServiceProps, openAPIData: OpenAPIObject) {
    ...
    if (this.config.hook?.afterOpenApiDataInited) {
      const openAPIData = this.config.hook.afterOpenApiDataInited(this.openAPIData);
      this.openAPIData = openAPIData || this.openAPIData;
    }
  }

  public genInterfaceFromTemplate = (name: string, type: TypescriptFileType, option: any) => {
    const module = {};
    option?.list?.forEach?.((item) => {
      let arr = item?.props?.[0] || [];

      const moduleName = extractData(item.typeName);

      if (item.typeName?.endsWith?.('ResponseVo')) {
        arr?.forEach?.((data, index) => {
          if (item.typeName === 'ResponseVo' && data.name === 'data') {
            item.generics = '<T=any>';
            data.type = 'T';
            item.props[0][index] = data;
          } else if (data.name === 'data') {
            const type = `${data?.type}`.replace('(', '').replace(')', '');
            item = {
              desc: data.desc,
              customType: type,
              content: `
                    /*${data.desc}*/
               type ${item.typeName} = ResponseVo<${type}>
               `,
            };
          }
        });
      }
      module[moduleName] = [...(module[moduleName] || []), item];
    });
    const prettierError = [];

    Object?.entries?.(module)?.forEach?.(([k, v]) => {
      const handErr = this.genFileFromTemplate(name, type, { ...option, list: v, pathname: k });
      prettierError.push(handErr);
    });
    if (prettierError.includes(true)) {
      Log(`🚥 格式化失败，请检查 typing.d.ts 文件内可能存在的语法错误`);
    }
  };

  public genServiceControllerFromTemplate = (
    name: string,
    template: TypescriptFileType,
    option: any,
  ) => {
    const prettierError = [];
    this.getServiceTP().map((item) => {
      const className = extractData(item.className);
      const handErr = this.genFileFromTemplate(name, template, {
        ...option,
        ...item,
        pathname: className,
      });
    ...
    if (prettierError.includes(true)) {
      Log(`🚥 格式化失败，请检查 api 文件内可能存在的语法错误`);
    }
  };
  ...
  public genFile() {
    ...
    // 生成 ts 类型声明
    this.genInterfaceFromTemplate('typings.d.ts', 'interface', {
    ...
    // 生成 controller 文件
    this.genServiceControllerFromTemplate('api.ts', 'serviceController', {
      namespace: this.config.namespace,
  ...
  public getServiceTP() {
    return Object.keys(this.apiData)
      .map((tag) => {
        ...
        let genParams = this.apiData[tag]
          .filter(
            (api) =>
              // 暂不支持变量
              !api.path.includes('${'),
          ...
          .map((api) => {
            ...
            try {
              ...
              const newfinalParams = finalParams as any;
              // 处理 query 中的复杂对象
              if (newfinalParams && newfinalParams.query) {
                newfinalParams.query = newfinalParams.query.map((ele) => ({
                  ...ele,
        ...
        if (genParams.length) {
          genParams = genParams.map((item) => ({ ...item, path: item.path.replace('/v1', '') }));
  ...
  private genFileFromTemplate(
    ...
    try {
      ...
      const serversPath = this.config.serversPath || './src/service';
      writeFile(
        serversPath + `/${params.pathname}`,
        fileName,
        nunjucks.renderString(template, params),
      );
    } catch (error) {
      // eslint-disable-next-line no-console
      console.error('[GenSDK] file gen fail:', error, fileName, 'type:', type);
      throw error;
    }
  }
