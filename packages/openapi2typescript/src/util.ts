...
export const writeFile = async (folderPath: string, fileName: string, content: string) => {
  const filePath = path.join(folderPath, fileName);
    ...
    prettierFile(content).then(([prettierContent, hasError]) => {
      if (hasError) {
        throw hasError;
      }
...
export const  extractData = (str: string) => {
  let regex = /[A-Z]([^A-Z]*)[A-Z]/;
  let matches = str.match(regex);
  
  if (!matches) return str.toLowerCase();
  else return matches[0].toLowerCase().substring(0, matches[0].length - 1);
};